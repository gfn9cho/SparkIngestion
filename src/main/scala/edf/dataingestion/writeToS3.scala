...
object writeToS3 {
  def insertInToS3(df: DataFrame , s3TempLocation: String, hiveTable: String,
    ...
    */
    ...
    def hudiOrSparkWrite(df: DataFrame): Unit = {
      stgLoadBatch match {
        ...
        case true =>
            s3BackUp(false)
            StageIncremental.identifyPartitions(df, hiveTable, batchPartition, isDeleteTable)
          df
            .withColumn("bucket", pmod(col("id"), lit(10)))
            .write.format("parquet")
            .partitionBy("ingestiondt", "bucket")
  ...
  def apply(df: DataFrame, s3TempLocation: String, hiveTable: String, saveMode: SaveMode,
            batchPartition: String, cdcColMax: String)
           (implicit  spark: SparkSession): (Long, Long) = {
      insertInToS3(df, s3TempLocation, hiveTable, saveMode, batchPartition, cdcColMax)
  }
}
