...
echo "Started the script for GWCL Delete Batch "
...
log_file=$base_dir"/logs/"$date"/gwclDeleteIngestion_"$date_ts".log"
...
spark-submit --class edf.dataingestion.DataLoad \
 ...
 --name "gwcl_delete_batch_dataingestion"  \
  --files ${spec_files_path}/gwcl_delete_batch.properties#diProperties.properties,${spec_files_path}/gwcl_lookup_info.csv#gwcl_lookup_info.csv,${spec_files_path}/gwcl_pii_spec.csv#gwcl_pii_spec.csv,${spec_files_path}/gwcl_table_spec.csv#gwcl_table_spec.csv,/etc/spark/conf/hive-site.xml \
 --properties-file /usr/lib/spark/conf/spark-defaults.conf \
 ${jar_path}/${spark_app_jar_file_suffix}-${spark_app_jar_file_version} \
gwcl_delete_batch_dataingestion 1>>$log_file 2>&1  
status=$?
echo "Completed the script and the status is $status"
if [ ${status} -eq 0 ]; then
	exit 0
else
	echo "Spark job failed with error status for GWCL Delete Batch, Please have a look at the logs in below file on the server in the appropriate s3 bucket for yarn logs"
exit 1
fi
