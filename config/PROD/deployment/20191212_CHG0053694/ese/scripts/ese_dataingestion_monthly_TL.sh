...
echo "Started the script for ESE monthly TL "
...
spark_app_jar_file_version=1.1.8.jar
date=`date +%Y-%m-%d`
date_ts=`date '+%Y-%m-%d_%H%M%S%s'`
mkdir -p $base_dir/logs/$date
log_file=$base_dir"/logs/"$date"/eseMonthlyTLdataingestion_"$date_ts".log"
...
if [ ${status} -eq 0 ]; then
	...
	echo "Spark job failed with error status for ESE monthly TL, Please have a look at  ${log_file} file for yarn logs"
	exit 1
fi
