...
echo "Started the script for ERO TL "
base_dir=/home/hadoop/edf
hadoop_base_dir=/user/hadoop/edf

spec_files_path=$base_dir/ero/spec_files
jar_path=$base_dir/jars
spark_app_jar_file_suffix=edf_dataingestion-assembly
spark_app_jar_file_version=1.1.4.jar
date=`date +%Y-%m-%d`
date_ts=`date '+%Y-%m-%d_%H%M%S%s'`
mkdir -p $base_dir/logs/$date
log_file=$base_dir"/logs/"$date"/eroTLdataingestion_"$date_ts".log"
...
spark-submit --class edf.dataingestion.DataLoad \
 ...
 --name "ERO_TL_DataIngestion"  \
  --files ${spec_files_path}/ero.properties#diProperties.properties,${spec_files_path}/ero_lookup_info.csv#ero_lookup_info.csv,${spec_files_path}/ero_pii_spec.csv#ero_pii_spec.csv,${spec_files_path}/ero_table_spec.csv#ero_table_spec.csv,/etc/spark/conf/hive-site.xml \
 --properties-file /usr/lib/spark/conf/spark-defaults.conf \
 ${jar_path}/${spark_app_jar_file_suffix}-${spark_app_jar_file_version} \
ERO_TL_DataIngestion  1>>$log_file 2>&1
status=$?
echo "Completed the script and the status is $status"
if [ ${status} -eq 0 ]; then
	exit 0
else
	echo "Spark job failed with error status for ERO TL, Please have a look at the logs in below file on the server in the appropriate s3 bucket for yarn logs"
exit 1
fi
