...
echo "Started the script for HOINSPECT_MTHLY TL "
...
log_file=$base_dir"/logs/"$date"/hoinspect_mthlydataingestion_"$date_ts".log"
...
if [ ${status} -eq 0 ]; then
	...
	echo "Spark job failed with error status for HOINSPECT_MTHLY TL, Please have a look at the logs in below file on the server in the appropriate s3 bucket for yarn logs"
exit 1
fi
