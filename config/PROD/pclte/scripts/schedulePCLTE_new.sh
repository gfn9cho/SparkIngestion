...
sh /home/hadoop/processed/pclte/scripts/pclte_dataingestion_di_new.sh
status=$?
if [ ${status} -eq 0 ]; then
	echo "Completed the DI script and the status is $status"
        echo "starting with Hard delete batch"
	sh /home/hadoop/processed/pclte/scripts/pclte_dataingestion_delete_new.sh
	status=$?
	if [ ${status} -eq 0 ]; then
		echo "Completed the hard delete batch script and the status is $status"
        	echo "starting with staging load"
		sh /home/hadoop/staging/pclte/scripts/pclte_staging_new.sh
		status=$?
	        if [ ${status} -eq 0 ]; then
	                echo "competed the pclte Staging layer"
        	        exit 0;
			else
