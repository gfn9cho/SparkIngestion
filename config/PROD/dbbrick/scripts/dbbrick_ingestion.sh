...
echo "Started the script for dbbrick ingestion load"
source /home/hadoop/conf/env_conf.properties
date=`date +%Y-%m-%d`
date_ts=`date '+%Y-%m-%d_%H%M%S%s'`
mkdir -p $ingestion_base_dir/logs/$date
log_file=$ingestion_base_dir"/logs/"$date"/dbbrickDataIngestion_"$date_ts".log"

spec_files_path=$ingestion_base_dir/dbbrick/spec_files
...
spark-submit --class edf.dataingestion.DataLoad \
 ...
 $ingestion_jar_path/$dbbrick_ingestion_jar_name \
dbbrick_DI_dataingestion 1>>$log_file 2>&1
status=$?
echo "Completed the script and the status is $status"
if [ ${status} -eq 0 ]; then
	exit 0
else
	echo "Spark job failed with error status for dbbrick DI, Please have a look at the logs in below file on the server in the appropriate s3 bucket for yarn logs"
exit 1
fi
