...
echo "Started the script for GWCL TL ingestion load"
source /home/hadoop/conf/env_conf.properties
date=`date +%Y-%m-%d`
date_ts=`date '+%Y-%m-%d_%H%M%S%s'`
mkdir -p $ingestion_base_dir/logs/$date
log_file=$ingestion_base_dir"/logs/"$date"/gwclTLIngestion_"$date_ts".log"
...
spark-submit --class edf.dataingestion.DataLoad \
 ...
 --name gwcl_TL_dataingestion  \
 --files $spec_files_path/gwcl_TL.properties#diProperties.properties,$spec_files_path/gwcl_lookup_info.csv#gwcl_lookup_info.csv,$spec_files_path/gwcl_pii_spec.csv#gwcl_pii_spec.csv,$spec_files_path/gwcl_table_spec.csv#gwcl_table_spec.csv,/etc/spark/conf/hive-site.xml \
 --properties-file /usr/lib/spark/conf/spark-defaults.conf \
 $ingestion_jar_path/$ingestion_jar_name \
gwcl_TL_dataingestion 1>>$log_file 2>&1
status=$?
