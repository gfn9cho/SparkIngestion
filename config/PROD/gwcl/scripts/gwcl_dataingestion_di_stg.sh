...
spark-submit --class edf.dataingestion.DataLoad \
 ...
 --name gwcl_DI_stage_dataingestion  \
 --files $spec_files_path/gwcl_DI_stg.properties#diProperties.properties,$spec_files_path/gwcl_lookup_info.csv#gwcl_lookup_info.csv,$spec_files_path/gwcl_pii_spec.csv#gwcl_pii_spec.csv,$spec_files_path/gwcl_table_spec_stg.csv#gwcl_table_spec_stg.csv,/etc/spark/conf/hive-site.xml \
 --properties-file /usr/lib/spark/conf/spark-defaults.conf \
 $ingestion_jar_path/$ingestion_gwcl_jar_name \
gwcl_DI_stage_dataingestion 1>>$log_file 2>&1
status=$?
