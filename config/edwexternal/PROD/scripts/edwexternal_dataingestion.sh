...
echo "Started the script for EDWExternal TL"
DATE=`date +%Y-%m-%d`
...
oozie_files_path=$base_dir/edwexternal/oozie
...
--master yarn --deploy-mode cluster  --conf spark.shuffle.spill=true \
 ...
 --conf spark.executor.memory=30g --conf spark.driver.memory=10g  \
