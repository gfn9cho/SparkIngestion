...
echo "Started the script for HOINSPECT Daily TL ingestion load"
source /home/hadoop/conf/env_conf.properties
date=`date +%Y-%m-%d`
date_ts=`date '+%Y-%m-%d_%H%M%S%s'`
mkdir -p $ingestion_base_dir/logs/$date
log_file=$ingestion_base_dir"/logs/"$date"/hoinspect_dailyTLDataIngestion_"$date_ts".log"
...
spark-submit --class edf.dataingestion.DataLoad \
 ...
 --name HOINSPECT_DAILY_TL_DataIngestion  \
 --files $spec_files_path/hoinspect_daily_TL.properties#diProperties.properties,$spec_files_path/hoinspect_daily_lookup_info.csv#hoinspect_daily_lookup_info.csv,$spec_files_path/hoinspect_daily_pii_spec.csv#hoinspect_daily_pii_spec.csv,$spec_files_path/hoinspect_daily_table_spec.csv#hoinspect_daily_table_spec.csv,/etc/spark/conf/hive-site.xml \
 --properties-file /usr/lib/spark/conf/spark-defaults.conf \
 $ingestion_jar_path/$ingestion_jar_name \
HOINSPECT_DAILY_TL_DataIngestion 1>>$log_file 2>&1
status=$?
echo "Completed the script and the status is $status"
if [ ${status} -eq 0 ]; then
	exit 0
else
	echo "Spark job failed with error status for HOINSPECT Daily TL, Please have a look at the logs in below file on the server in the appropriate s3 bucket for yarn logs"
exit 1
fi
