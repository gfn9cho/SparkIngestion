...
spark-submit --class edf.dataingestion.DataLoad \
 ...
 --name gwpl_DI_stage_dataingestion  \
 --files $spec_files_path/gwpl_DI_stg.properties#diProperties.properties,$spec_files_path/gwpl_lookup_info.csv#gwpl_lookup_info.csv,$spec_files_path/gwpl_pii_spec.csv#gwpl_pii_spec.csv,$spec_files_path/gwpl_table_spec_stg.csv#gwpl_table_spec_stg.csv,/etc/spark/conf/hive-site.xml,$spec_files_path/fairScheduler.xml#fairScheduler.xml \
 --properties-file /usr/lib/spark/conf/spark-defaults.conf \
 $ingestion_jar_path/$ingestion_jar_name \
gwpl_DI_stage_dataingestion 1>>$log_file 2>&1
status=$?
