...
sh /home/hadoop/edf/pcagg/scripts/pcagg_dataingestion_di.sh
status=$?
if [ ${status} -eq 0 ]; then
	echo "Completed the first DI script and the status is $status"
	...
	if [ ${status} -eq 0 ]; then
		...
		sh /home/hadoop/edf/pcagg/scripts/pcagg_dataingestion_delete.sh
		status=$?
	        if [ ${status} -eq 0 ]; then
	                echo "Completed the hard delete batch script and the status is $status"
        	        echo "starting with staging load"

			sh /home/hadoop/staging/pcagg/pcagg_staging.sh
			status=$?
		        if [ ${status} -eq 0 ]; then
        ...
        echo "Spark job failed with error status for 1st DI load"
	exit 1
fi
