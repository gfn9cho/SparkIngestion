...
echo "Started the script for RAVEHICLE_MTHLY DI "
base_dir=/home/hadoop/edf
hadoop_base_dir=/user/hadoop/edf
spec_files_path=$base_dir/ravehicle/spec_files
...
log_file=$base_dir"/logs/"$date"/ravehicle_mthlyDIdataingestion_"$date_ts".log"
...
spark-submit --class edf.dataingestion.DataLoad \
 ...
 --name "RAVEHICLE_MTHLY_DI_dataIngestion"  \
  --files ${spec_files_path}/ravehicle_mthly_DI.properties#diProperties.properties,${spec_files_path}/ravehicle_mthly_lookup_info.csv#ravehicle_mthly_lookup_info.csv,${spec_files_path}/ravehicle_mthly_pii_spec.csv#ravehicle_mthly_pii_spec.csv,${spec_files_path}/ravehicle_mthly_table_spec.csv#ravehicle_mthly_table_spec.csv,/etc/spark/conf/hive-site.xml \
 --properties-file /usr/lib/spark/conf/spark-defaults.conf \
 ${jar_path}/${spark_app_jar_file_suffix}-${spark_app_jar_file_version} \
RAVEHICLE_MTHLY_DI_dataIngestion  1>>$log_file 2>&1
status=$?
echo "Completed the script and the status is $status"
if [ ${status} -eq 0 ]; then
	exit 0
else
	echo "Spark job failed with error status for RAVEHICLE_MTHLY DI, Please have a look at the logs in below file on the server in the appropriate s3 bucket for yarn logs"
exit 1
fi
