...
echo "Started the script for GWPL TL ingestion load"
source /home/hadoop/conf/env_conf.properties
date=`date +%Y-%m-%d`
date_ts=`date '+%Y-%m-%d_%H%M%S%s'`
mkdir -p $ingestion_base_dir/logs/$date
log_file=$ingestion_base_dir"/logs/"$date"/gwplTLIngestion_"$date_ts".log"

spec_files_path=$ingestion_base_dir/gwpl_adhoc_missingid/spec_files
...
spark-submit --class edf.dataingestion.DataLoad \
 ...
 --files $spec_files_path/gwpl_TL.properties#diProperties.properties,$spec_files_path/gwpl_lookup_info_TL.csv#gwpl_lookup_info_TL.csv,$spec_files_path/gwpl_pii_spec_TL.csv#gwpl_pii_spec_TL.csv,$spec_files_path/gwpl_table_spec_TL.csv#gwpl_table_spec_TL.csv,/etc/spark/conf/hive-site.xml \
