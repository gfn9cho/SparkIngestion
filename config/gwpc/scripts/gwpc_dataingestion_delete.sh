...
--master yarn --deploy-mode cluster  --conf spark.shuffle.spill=true \
 ...
 --conf spark.ui.port=8088 --conf spark.executor.memoryOverhead=4g  \
 ...
 --conf spark.executor.memory=32g --conf spark.driver.memory=20g  \
 --conf spark.executor.instances=2 --conf spark.serializer=org.apache.spark.serializer.KryoSerializer  \
 --name gwpc_delete_batch \
 --files $spec_files_path/gwpc_delete.properties#diProperties.properties,$spec_files_path/gwpc_lookup_info.csv#gwpc_lookup_info.csv,$spec_files_path/gwpc_pii_spec.csv#gwpc_pii_spec.csv,$spec_files_path/gwpc_table_spec.csv#gwpc_table_spec.csv,/etc/spark/conf/hive-site.xml \
