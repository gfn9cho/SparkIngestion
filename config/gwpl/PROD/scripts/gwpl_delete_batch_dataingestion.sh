...
echo "Started the script for GWPL Delete Batch "
...
log_file=$base_dir"/logs/"$date"/gwplDeletedataingestion_"$date_ts".log"
...
spark-submit --class edf.dataingestion.DataLoad \
 ...
 --name "gwpl_delete_batch_dataingestion"  \
  --files ${spec_files_path}/gwpl_delete_batch.properties#diProperties.properties,${spec_files_path}/gwpl_lookup_info_CI.csv#gwpl_lookup_info_CI.csv,${spec_files_path}/gwpl_pii_spec_CI.csv#gwpl_pii_spec_CI.csv,${spec_files_path}/gwpl_table_spec_CI.csv#gwpl_table_spec_CI.csv,/etc/spark/conf/hive-site.xml \
 --properties-file /usr/lib/spark/conf/spark-defaults.conf \
 ${jar_path}/${spark_app_jar_file_suffix}-${spark_app_jar_file_version} \
gwpl_delete_batch_dataingestion 1>>$log_file 2>&1
status=$?
echo "Completed the script and the status is $status"
if [ ${status} -eq 0 ]; then
	exit 0
else
	echo "Spark job failed with error status for GWPL Delete Batch, Please have a look at the logs in below file on the server in the appropriate s3 bucket for yarn logs"
exit 1
fi
