...
--master yarn --deploy-mode cluster  --conf spark.shuffle.spill=true \
 ...
 --name gwpc_di_dataingestion \
 --conf spark.executor.instances=1 --conf spark.serializer=org.apache.spark.serializer.KryoSerializer  \
 --files $spec_files_path/gwpc_di.properties#diProperties.properties,$spec_files_path/gwpc_lookup_info.csv#gwpc_lookup_info.csv,$spec_files_path/gwpc_pii_spec.csv#gwpc_pii_spec.csv,$spec_files_path/gwpc_table_spec.csv#gwpc_table_spec.csv,/etc/spark/conf/hive-site.xml \
 --properties-file /usr/lib/spark/conf/spark-defaults.conf \
 $jar_path/$spark_app_jar_file_suffix-$spark_app_jar_file_version \
 gwpc_di_dataingestion
