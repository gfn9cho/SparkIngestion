...
import edf.dataingestion.{connectDBInd, oldConsolidateTypeList, ref_col_list, splitString, targetDB}
  ...
  def makeJoins(df: DataFrame, typeTableList: List[(String, Int)])(implicit spark: SparkSession): DataFrame = {
    typeTableList match {
      ...
      case types :: tail => {
        ...
        val typeDF = if (connectDBInd == "Y")
          spark.table(oldConsolidateTypeList)
