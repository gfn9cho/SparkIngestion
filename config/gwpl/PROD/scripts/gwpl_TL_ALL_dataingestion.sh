...
echo "Started the script for GWPL TL "
...
log_file=$base_dir"/logs/"$date"/gwplTLALLdataingestion_"$date_ts".log"
...
spark-submit --class edf.dataingestion.DataLoad \
 ...
 --name "GWPL_TL_ALL_DataIngestion"  \
  --files ${spec_files_path}/gwpl_TL_ALL.properties#diProperties.properties,${spec_files_path}/gwpl_lookup_info_TL_ALL.csv#gwpl_lookup_info_TL_ALL.csv,${spec_files_path}/gwpl_pii_spec_TL_ALL.csv#gwpl_pii_spec_TL_ALL.csv,${spec_files_path}/gwpl_table_spec_TL_ALL.csv#gwpl_table_spec_TL_ALL.csv,/etc/spark/conf/hive-site.xml \
 --properties-file /usr/lib/spark/conf/spark-defaults.conf \
 ${jar_path}/${spark_app_jar_file_suffix}-${spark_app_jar_file_version} \
GWPL_TL_ALL_DataIngestion 1>>$log_file 2>&1
status=$?
