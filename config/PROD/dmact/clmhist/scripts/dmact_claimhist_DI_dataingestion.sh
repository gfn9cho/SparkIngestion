...
echo "Started the script for DMACT CLAIM HIST ingestion load"
source /home/hadoop/conf/env_conf.properties
date=`date +%Y-%m-%d`
date_ts=`date '+%Y-%m-%d_%H%M%S%s'`
mkdir -p $ingestion_base_dir/logs/$date
log_file=$ingestion_base_dir"/logs/"$date"/dmactclaimhistDataIngestion_"$date_ts".log"

spec_files_path=$ingestion_base_dir/dmact/clmhist/spec_files
...
if [ ${status} -eq 0 ]; then
	...
	echo "Spark job failed with error status for DMACT CLAIM HIST, Please have a look at the logs in below file on the server in the appropriate s3 bucket for yarn logs"
exit 1
fi
