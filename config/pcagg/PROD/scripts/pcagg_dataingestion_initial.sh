...
echo "Started the script for PCAGG TL"
...
spark-submit --class edf.dataingestion.DataLoad \
 ...
 --name "PCAGG_TL_DataIngestion"  \
 --files ${spec_files_path}/pcagg_initial.properties#diProperties.properties,${spec_files_path}/pcagg_lookup_info.csv#pcagg_lookup_info.csv,${spec_files_path}/pcagg_pii_spec.csv#pcagg_pii_spec.csv,${spec_files_path}/pcagg_table_spec.csv#pcagg_table_spec.csv,/etc/spark/conf/hive-site.xml \
 --properties-file /usr/lib/spark/conf/spark-defaults.conf \
 ${jar_path}/${spark_app_jar_file_suffix}-${spark_app_jar_file_version} \
 PCAGG_TL_DataIngestion  
status=$?
echo "Completed the script and the status is $status"
if [ ${status} -eq 0 ]; then
	exit 0
else
	echo "Spark job failed with error status for PCAGG TL, Please have a look at the logs in below file on the server in the appropriate s3 bucket for yarn logs"
exit 1
fi
