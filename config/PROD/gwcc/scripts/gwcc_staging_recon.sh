...
echo "Started the script for GWCC 34DI Staging "
...
log_file=$base_dir"/logs/"$date"/gwccReconIncr_"$date_ts".log"
...
spark-submit --class edf.recon.ReconJob \
 ...
 --name "gwcc_recon_staging_incremental"  \
...
gwcc_recon_staging_incremental  1>>$log_file 2>&1
status=$?
