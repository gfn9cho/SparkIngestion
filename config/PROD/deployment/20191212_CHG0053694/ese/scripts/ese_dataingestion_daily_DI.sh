...
echo "Started the script for ESE DI "
...
log_file=$base_dir"/logs/"$date"/eseDIdataingestion_"$date_ts".log"
...
if [ ${status} -eq 0 ]; then
	...
	echo "Spark job failed with error status for ESE DI, Please have a look at  ${log_file} file for yarn logs"
exit 1
fi
