...
echo "Started the script for GWBC CI ingestion load"
source /home/hadoop/conf/env_conf.properties
date=`date +%Y-%m-%d`
date_ts=`date '+%Y-%m-%d_%H%M%S%s'`
mkdir -p $ingestion_base_dir/logs/$date
log_file=$ingestion_base_dir"/logs/"$date"/gwbcCIIngestion_"$date_ts".log"
...
spark-submit --class edf.dataingestion.DataLoad \
 ...
 --name gwbc_CI_dataingestion  \
 --files $spec_files_path/gwbc_CI.properties#diProperties.properties,$spec_files_path/gwbc_lookup_info.csv#gwbc_lookup_info.csv,$spec_files_path/gwbc_pii_spec.csv#gwbc_pii_spec.csv,$spec_files_path/gwbc_table_spec.csv#gwbc_table_spec.csv,/etc/spark/conf/hive-site.xml \
 --properties-file /usr/lib/spark/conf/spark-defaults.conf \
 $ingestion_jar_path/$ingestion_jar_name \
gwbc_CI_dataingestion 1>>$log_file 2>&1
status=$?
