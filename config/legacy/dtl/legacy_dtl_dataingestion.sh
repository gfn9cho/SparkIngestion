...
echo "Started the DTL script for Legacy source"
base_dir=/home/hadoop/edf
hadoop_base_dir=/user/hadoop/edf
spec_files_path=$base_dir/legacy/dtl
...
Legacy_DTL_DataIngestion 

status=$?
echo "Completed the DTL script for Legacy and the status is $status"
if [ ${status} -eq 0 ]; then
	exit 0
else
	echo "Spark job for Legacy DTL failed with error status, Please have a look at the logs in s3 path s3a://sa-l4f-emr-edl/logs"
exit 1
fi
